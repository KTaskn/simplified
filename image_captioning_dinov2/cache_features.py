"""Pre-compute and cache DINOv2 image features for faster training."""
...
class CocoImageDataset(Dataset):
    ...
    def __init__(self, image_ids: list[int], id_to_filename: dict[int, str],
                 image_dir: str, image_processor: AutoImageProcessor):
...
def cache_features(cfg: Config, split: str, batch_size: int = 256):
    ...
    vision_model = Dinov2Model.from_pretrained(cfg.encoder_model_name).to(device)
    vision_model.eval()

    image_processor = AutoImageProcessor.from_pretrained(cfg.encoder_model_name)

    dataset = CocoImageDataset(image_ids, id_to_filename, cfg.coco_image_dir, image_processor)
    loader = DataLoader(
        dataset,
    ...
    with torch.no_grad():
        for batch_ids, pixel_values in tqdm(loader, desc=f"Caching {split}"):
            ...
            features = output.last_hidden_state.half().cpu()  # (B, 257, 384) in float16
