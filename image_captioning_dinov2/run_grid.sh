#!/bin/bash
# Grid search: model_size x vocab_size (DINOv2 version)
...
echo "=== Grid Search: ${VARIANT} (DINOv2) ==="
echo "  Model sizes: ${MODEL_SIZES[*]}"
echo "  Vocab sizes: ${VOCAB_SIZES[*]}"
echo "  Total runs: $(( ${#MODEL_SIZES[@]} * ${#VOCAB_SIZES[@]} ))"

# Step 0: Cache DINOv2 features (one-time, shared across all runs)
echo ""
echo "########################################"
echo "  Step 0: Caching DINOv2 features"
echo "########################################"
...
# Results are automatically appended to outputs_dinov2/grid_results_{variant}.csv by train.py
echo ""
echo "########################################"
...
for SIZE in "${MODEL_SIZES[@]}"; do
    for VSIZE in "${VOCAB_SIZES[@]}"; do
        echo ""
        echo "======================================"
        echo "  Training: ${VARIANT} / ${SIZE} / v${VSIZE} (DINOv2)"
        echo "======================================"
        ...
        CKPT="$(dirname "$(pwd)")/outputs_dinov2/${RUN_NAME}/checkpoints/best.pt"

        if [ -f "$CKPT" ]; then
            echo ""
            echo "======================================"
            echo "  Evaluating: ${VARIANT} / ${SIZE} / v${VSIZE} (DINOv2)"
            echo "======================================"
...
RESULTS_CSV="$(dirname "$(pwd)")/outputs_dinov2/grid_results_${VARIANT}.csv"
echo ""
echo "======================================"
echo "  Grid Search Complete: ${VARIANT} (DINOv2)"
echo "======================================"
