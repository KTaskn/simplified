"""Training loop for Japanese Image Captioning model (DINOv2 version)."""
...
def find_max_batch_size(
    ...
    cfg: Config,
    use_cache: bool,
    cache_sample_shape: tuple | None = None,
    min_bs: int = 256,
    max_bs: int = 32768,
    ...
    def try_batch(bs: int) -> bool:
        ...
        try:
            model.train()
            if not use_cache and model.vision_encoder is not None:
                model.vision_encoder.eval()

            if use_cache:
                # Use actual cache shape (num_patches, embed_dim)
                num_patches, embed_dim = cache_sample_shape
                dummy_img = torch.randn(bs, num_patches, embed_dim, device=device)
            else:
...
def main():
    ...
    parser.add_argument("--use_cache", action="store_true", help="Use pre-cached DINOv2 features")
    ...
    print(f"Encoder: DINOv2 ({cfg.encoder_model_name})")
    ...
    cache_sample_shape = None
    if use_cache:
        cache_dir = Path(cfg.feature_cache_dir)
        ...
        if not train_cache.exists() or not val_cache.exists():
            ...
            return
        print("Loading datasets (cached DINOv2 features)...")
        ...
        # Get cache sample shape for auto batch size
        sample_key = next(iter(train_dataset.cache))
        cache_sample_shape = train_dataset.cache[sample_key].shape
        print(f"Cache feature shape: {cache_sample_shape}")
    ...
    if args.auto_batch and device.type == "cuda":
        ...
        optimal_bs = find_max_batch_size(model, device, cfg, use_cache, cache_sample_shape)
